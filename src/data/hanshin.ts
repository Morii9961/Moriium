...
/**
 * Copy and route helpers for the 「版心」 design study.
 *
 * The study keeps its own strings instead of widening src/data/site.ts, so the
 * production interface copy stays exactly as it is while this direction is
 * still a proposal. Shared strings (search dialog, theme labels) are read from
 * site.ts rather than duplicated here.
 */

export const HANSHIN_BASE = '/design/hanshin';

export function hsHome(lang: Language) {
  return `${HANSHIN_BASE}/${lang}/`;
}

export function hsPath(lang: Language, ...segments: string[]) {
  const tail = segments.map((segment) => encodeURIComponent(segment)).join('/');
  return tail ? `${HANSHIN_BASE}/${lang}/${tail}/` : hsHome(lang);
}

export const HANSHIN = {
  zh: {
...
export type HanshinCopy = (typeof HANSHIN)[Language];

/**
 * Dates in the annotation register are numeric and language-independent, so
 * they set in the monospace face and align in a column. Prose keeps the
 * localised long form from utils/content.
 */
export function hsDay(date: Date) {
  return new Intl.DateTimeFormat('en-CA', { year: 'numeric', month: '2-digit', day: '2-digit' }).format(date);
}

export function hsYear(date: Date) {
  return String(date.getFullYear());
}

/** Reading estimate that counts han and kana per character, Latin per word. */
export function hsReadingMinutes(body: string) {
...
export function hsCount<T>(items: T[], key: (item: T) => string[]) {
