...
> 状态：**B Hybrid 路线已由 Morii 选定；生产实现仍待 Phase 5 ADR 批准**
> 日期：2026-08-29；决策更新：2026-08-30
> 适用范围：公开站、写作工具、内容模型、媒体管线与部署边界  
> 当前生产约束仍以 [`AGENTS.md`](../AGENTS.md) 和 [`architecture.md`](architecture.md) 为准。

## 结论先行

Morii 已实际使用原型 B，并于 2026-08-30 将其选为 Moriium vNext 的最终路线：登录、版本化数据库、常驻服务和全栈 Admin 可以进入生产架构。原型 A 不再开发。

> **已选路线是 Experience-first Hybrid：全栈 Admin、认证、数据库和 API 负责写作与发布；公开文章优先预渲染或缓存，只有登录态、权限草稿、即时内容等确有体验收益的路由按需渲染。**
...
作者登录是 Web Admin 的必要组成；公开读者账户则必须对应明确价值，例如跨设备书签、评论、订阅偏好或受控内容，不能只因为技术上能做就加入。B Hybrid 已经选定；C 全站 SSR 仍只能由后续 Hybrid 运行证据、代表性路由实验和第二份 ADR 决定。
...
| --- | --- | --- | --- | --- | --- | --- |
| A. 静态站 + 本地 Studio | Astro SSG | Markdown | Git + CI | 运维最轻；内容保真与隐私边界清楚 | 远程写作、即时预览和发布体验较弱 | **已取消，不再开发原型** |
| B. 全栈 Admin + 混合公开站 | Astro 逐路由预渲染/按需渲染 | DB 中的版本化 canonical content | Admin 发布 + 重验证/构建/缓存失效 | 接近 ohmyblog 的后台体验；公开阅读仍可保持静态速度与降级能力 | 需要认证、权限、迁移、备份、缓存和监控 | **Morii 已选为 vNext 路线** |
| C. 全栈 Admin + 全站 SSR | Astro server | DB 中的版本化 canonical content | 请求时读取 | 所有内容与个性化状态即时 | 故障域、缓存和公开阅读可用性成本最大 | 只有实测优于 B 才选择 |

### B 的选择结果

Morii 实际使用 B 后确认满意，并决定不再开发 A。这个结果来自 Morii 对 B 的直接体验，不是完整的 A/B 量化比较；文档不得倒推成“A 经测试失败”。原计划的比较项中仍有价值的内容会转成 B 的验收清单。
...
本阶段原计划使用同一套虚构 fixture 做两个隔离原型。B 已经完成可操作闭环；Morii 于 2026-08-30 根据实际体验选定 B，并取消 A：

1. ~~A：本地 Markdown 编辑 + frontmatter 表单 + 生产同源预览；~~ **取消，不再开发**；
2. B：单一作者登录 + Tiptap 编辑 + 自动保存 + 数据库存储 + 权限草稿预览 + 测试发布，**已选为 vNext 路线**。

原计划的任务时间、点击数与维护成本对照不再执行。内容保真、三语与图片处理、自动保存/冲突恢复、预览一致性、发布延迟和安全仍作为 B 的验收项。

退出条件：B 补齐完整发布闸门、图片属性面板和生产同源草稿预览；所有 fixture 仍可由现有站点安全渲染。C 全站 SSR 不在本阶段选择。
...
### Phase 5 — B Hybrid 生产架构 ADR

路线已经选定。用 Phase 0–4 的原型数据和 Morii 的实际操作固定 B 的生产形态：
...
ADR 不再重开 A/B 选择，只为 B 固定初始逐路由 Hybrid 策略，并更新 `AGENTS.md`、部署合同、威胁模型和回退方案。C 只能在 Phase 6A 运行证据、Phase 6B 实验和第二份扩围 ADR 后成为生产候选。
...
Morii 已确认：B Hybrid 是 vNext 最终路线，全栈转型、作者登录、版本化数据库和增加开发/运维工作量都可以接受；ohmyblog 的后台体验仍是重要参照。原型 A 不再开发。
...
Morii 本次确认结束了路线选择，但不等于批准把当前尖峰直接改造成生产。B 仍需补齐完整发布闸门、图片属性面板和生产同源草稿预览；生产迁移仍须由 Phase 5 ADR 批准，并同步更新项目合同。
