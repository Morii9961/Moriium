...
完整记录在 ADR 13.10–13.20，这里只给接手需要的骨架。4.5 与 4.6 是 Codex 在 2026-08-30 做的，那一轮额度耗尽没来得及写文档，ADR 13.15、13.16 由 Claude 依据提交内容与重跑的检查补记；4.7 与 4.8 接在后面。
...
pnpm -C prototypes test               # 117 个用例
...
16. **`![]()` 修好之前写进库的草稿仍然带着它。**13.18 那个填充图片从 13.13 起就在往正文里加，修的是以后不再加，**不会回头清理已经保存的版本**。Morii 那个跑了一上午的 `admin-b/.data/admin.db` 很可能已经有几版带 `![]()` 的自动保存。**闸门原本也拦不住它**：图片路径的正则要求至少一个字符，而 `![]()` 的路径是空的，根本不匹配，所以那三个版本全部通过了发布闸门。这一条已修（正则改成允许空路径，并单独给一句错误信息），但**已经存进库里的版本仍然带着它**，要么删库重播种，要么手工删掉那一行。
17. **自动保存失败之后不会自动重试。**API 断不上时正文不会丢，状态也不会谎称已保存，界面还会说明改动只在编辑器里（13.20 已改）。但它只在下一次编辑时才再排一次保存，所以**作者停手不打字，就永远不会再试**。加退避重试要连着「重试期间显示什么、几次以后放弃」一起定，是界面决定，没有顺手做。
18. **窄屏下右栏面板要滚很久才够得到。**375 像素下版本历史、预览与读者面板落在正文之后，长文里大约 3100 像素以下。不是溢出，是够不着，属于界面决定。
19. **移动模拟下自动化点不动。**合成触摸点击会 30 秒超时，这是自动化工具的限制，不是应用缺陷；B1 的回车提交没验过也卡在这里。真实浏览器里请 Morii 自己确认。
...
| | 项 |
| --- | --- |
| 已通过 | B3 内容块保真、B4 中断恢复、B5 预览同源、B8 发布回滚、B9 375 像素 |
| 部分 | B1 缺回车提交、B6 缺翻译状态界面、B10 失败后不重试、B11 缺渲染出来的公开页 |
| 不能过 | B2 存不下完整 frontmatter、B7 没有媒体导入路径 |
| 未验 | 无 |

**Phase 5 的 ADR 已获批准**（2026-08-30）：[`adr-0002-phase5-production.md`](adr-0002-phase5-production.md)。Morii 在 2026-08-30 定了四件事——只做作者账户但建两个（Morii、Enouia）、发布分钟级可见、Admin 的暴露面、后端用 Astro 内置——ADR 从这四条推出其余部分，并把 B2 与 B7 的修法一并定在里面。Admin 那条改过三轮：隧道 → 客户端证书 → **直接公网开放**，最终靠长随机口令、按账户限速和 fail2ban 防守，接受的代价写在 ADR 第 10.4 节。
