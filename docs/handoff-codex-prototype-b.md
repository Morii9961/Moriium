...
**Phase 5 的 ADR 已获批准**（2026-08-30）：[`adr-0002-phase5-production.md`](adr-0002-phase5-production.md)。Morii 在 2026-08-30 定了四件事——只做作者账户但建两个（Morii、Enouia）、发布分钟级可见、Admin 用客户端证书保护、后端用 Astro 内置——ADR 从这四条推出其余部分，并把 B2 与 B7 的修法一并定在里面。Admin 那条先定的是隧道，Morii 嫌每次要先连，改成 mTLS：日常零操作，陌生人同样到不了登录页。
...
批准时 Morii 同时定了两条减法：**暂不做告警**（第 12 节因此降级为「可以查，不会通知你」，残余风险写在 12.2），**不做移动端证书**（Admin 只在桌面设备上用）。ADR 第 18 节剩下的三项都不需要 Morii 拍板。
