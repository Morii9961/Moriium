...
| B4 | 自动保存后模拟中断并恢复 | 杀掉进程重开，最后一次自动保存的正文还在，且读者看到的内容没变过 | **未验**——版本表支持它，没有实际演练过 |
...
| B9 | 在 375 CSS 像素宽度下完成一次小修改 | 编辑器、图片属性面板、预览面板在 375 像素下可操作，不横向溢出 | **未验**——一次都没在窄屏下开过 |
| B10 | 故障演练 | 数据库锁与 API 断开时给出可理解的失败，不静默丢改动 | **未验**——`db-locked` 已建模为可重试，没演练过 |
...
部分：B1（缺回车提交）、B6（缺界面）、B11（缺渲染出来的公开页）。

不能过：B2、B7。

一次都没验过：B4、B9、B10。

**B 被选中，不等于 B 通过了这份清单。**十一项里现在只有三项拿得出完整证据。下一步是把 B4、B9、B10 这三项实际演练一遍——它们不需要新功能，只需要有人真的去做，做完清单的空白就只剩下范围边界那几项。
