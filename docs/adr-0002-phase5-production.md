...
### 21.20 第 00 节的体积与构建基线（2026-08-31）

06A 的故障矩阵结束后，下一步本应是影子部署，但 VPS 尚未购置。因此本轮转去补第 00 节一直空着的一项：把「vNext 不能退步的基线」从一句待办变成可复跑、会自己红的检查。

基线不写成文档里的数字。文档里的数字没人会重新测，也不会失败。所以测量口径和预算写在同一个命令里，由 `scripts/measure-baseline.mjs` 承担，并接进 `pnpm verify` 的末尾（构建之后）。

固定口径，两次运行才可比：

- 只统计公开树。`dist/server/` 不是读者会取的东西，第 4 节把它排除在读者路径之外。
- 「eager」指 HTML 里由 `src=`/`href=` 直接引用的资源。动态 `import()` 按定义不计入——这条排除本身就是要证明的东西。
- 同时报原始体积与 gzip -6，因为 `deploy/nginx/moriium.conf` 实际发出的就是 level 6。
- `/design/` 研究树单独计量并排除在生产总量之外。它是没有视觉授权的隔离研究，churn 不该移动生产基线。
- **排除必须跟着资源走，不能只按页面路径切。**这一条是本节最初写错的地方，单独记在下面。

#### 资源归属：本节第一版的数字是错的

第一版按路径切：`design/` 下的文件算研究，其余算生产。据此记下「生产 26.9 MB、研究 0.8 MB」。这个数字是错的。

研究页面确实住在 `design/` 下，但它们引用的样式表和字体文件和其他页面一样落进共享的 `_astro/`。只按路径切，等于把研究独有的每一个字节都算进了生产总量——写下这一节时那是 226 个文件、12.7 MB，约等于当时所报生产重量的一半。

改成按可达性归属：`_astro/` 下的资源看谁引用它。生产页面能到达的算生产；只有研究页面能到达的（直接引用，或经由另一个资源间接引用）算研究；两边都到不了的留在生产总量里并单独报出来，因为公开树里出现孤儿资源本身就是值得看见的事实。

「间接」是这里的关键。真实的形状是：研究页面引用一张样式表，只有那张样式表引用字体文件。只读页面、或只按路径切，都会漏掉真正占体积的那部分。

复跑后的实测，代码为 `3151ff0` 加上工作树里未提交的 Hanshin 与 Jiege 研究改动：

```text
pnpm build            → exit 0，耗时 8s
pnpm baseline         → exit 0

生产文件            413 个
生产页面            33 个 HTML
生产体积            14.2 MB
设计研究            51 个 HTML / 13.6 MB（不计入）
研究独有资源        229 个，位于 _astro/ 下（不计入）
无引用资源          6 个，位于 _astro/ 下（计入生产）

每页首次渲染前：
  首页 zh/ja/en        js   1.0 KB   css 264.5 KB（91.8 KB gz）
  Writing 索引         js   1.0 KB   css 264.5 KB（91.8 KB gz）
  普通文章             js   1.0 KB   css 264.5 KB（91.8 KB gz）
  能力验收文章         js   7.5 KB   css 321.8 KB（105.9 KB gz）

搜索索引            3 种语言，最大 0.3 KB gz（打开搜索时才取）
```

生产体积从 26.9 MB 降到 14.2 MB，不是构建变小了，是先前把研究的字节算进了生产。设计研究从 0.8 MB 涨到 13.6 MB 是同一件事的另一半。

**只有生产那几个数是基线，研究那几个不是。**Hanshin 与 Jiege 两个研究还在并行开发，同一次会话里重测，研究树就从 51 个 HTML / 13.6 MB 变成 83 个 / 13.8 MB；而生产在两次测量之间稳定在 413 个文件 / 33 个 HTML / 14.2 MB 不动。这正是这项排除要达到的效果：研究怎么长，生产基线都不动。引用研究数字时要带上测量时刻，不要当成会保持的值。

两个结论值得单独记：

**`AGENTS.md` 那条「普通页面不得下载高级模块」现在有了体积证据。**普通页面只引用一个 `BaseLayout` 脚本，1.0 KB。Mermaid、cytoscape（0.41 MB）、KaTeX（0.25 MB）、PhotoSwipe 都不在引用里；能力验收文章会拉起 KaTeX 与 PhotoSwipe 的样式和 7.5 KB 脚本，Mermaid 仍然保持懒加载。原有的 marker 规则从结构上保证这件事，体积预算补的是 marker 管不到的那种情况：打包器把某个高级模块并进公共 chunk。

**关键路径现在几乎全是字体 CSS。**每页 264.5 KB CSS 里，`lxgwwenkaigbscreen.css` 占 201.7 KB，gzip 后 78.5 KB——是读者每页实际下载量的 87%。它是一张按 `unicode-range` 切成 439 个 WOFF2 分片的 CJK 字体声明表，字体文件本身仍按需取，大的是声明块。如果将来页面重量成为问题，这是唯一明显的杠杆。

预算故意不齐平，因为它们支撑的主张强度不同。普通页面 eager JS 的 8 KB 是严格的那条：它要拦的最小模块也比它大一个量级。其余三条是带余量的记录值，作用是让增长变得可见且需要说明，不是卡紧。

按 ADR 0001 第 5 节，检查必须被看着红过一次。`tests/public-baseline.test.mjs` 用 `--root` 指向合成树跑真实命令，九个用例：超预算退出 1 并指名是哪一条、`--report` 只报不失败、能力文章的预算与普通页面互不串、样本页面缺失算失败而不是干净通过、严格预算远低于任一高级模块。

资源归属另有三个用例把边界钉死，合成树复现真实形状——研究页面引用样式表，只有样式表引用字体：

- 只有研究能到达的资源被排除；
- **一旦有任何一个生产页面也引用它，立刻算回生产。**这是负向的那条，也是这项排除敢存在的理由：没有它，把重资源挪到研究引用后面就成了让真实生产重量凭空消失的办法；
- 两边都到不了的资源留在生产总量里并被报出来，不静默丢弃。

写负向测试时当场查出脚本两个缺陷：页面缺失时 `eagerAssets` 直接抛异常，那条「missing from the build」分支根本到不了；以及上面那条按路径切的归属错误。都已修。

```text
node --test --test-isolation=none tests/public-baseline.test.mjs
  → tests 9 / pass 9 / fail 0

node --test --test-isolation=none tests/*.test.mjs
  → tests 163 / pass 162 / fail 0 / skip 1（Windows symlink）

pnpm check → 0 errors    pnpm links / audit / split → 全部 exit 0
```

这一节只是本机测量，不涉及任何远端。

### 21.21 采购、部署、安全与恢复验收清单（2026-08-31）

第 00 节另一项「为公网 Admin、数据库、备份和常驻服务列出可验收的安全与恢复要求」由 [`docs/vps-acceptance-checklist.md`](vps-acceptance-checklist.md) 完成。它不新增任何决定，只把本 ADR 已定的东西整理成可逐条打勾的形式：编号、要求、验收方法、通过标准，以及来自哪一节。通过标准只有通过与未通过，确实还不能验的条目集中在末节并写明在等什么。

清单同时充当采购规格。默认下限定为 **2 核、4 GB 内存、80 GB SSD**；目标机上的峰值 RSS、磁盘占用与构建耗时作为购买之后的验收与升级依据，而不是购买之前的猜测。

三条边界写进清单本身：

- **主域名只在影子验收全部通过之后才切过去。**影子阶段的探测一律走临时主机名或本机 `hosts` 覆盖，不动 `morii9961.top` 的解析。把域名先切过去再验收，等于拿正式站做实验。
- **所有破坏性验收只在隔离测试库、fixture 账户和生成的虚构 EXIF 图片上做。**连续发布 7 次、人为让上线前检查失败、把 `sanitized_at` 改回 NULL、连续失败登录，这些都会改状态；它们一律不得碰正式数据库、正式内容或原始照片。
- **日志检查不得把可能存在的秘密打印出来。**只报命中条数与通过与否，不回显匹配行——一条为了证明日志干净而把口令打进终端的命令，本身就是泄露。

现有阈值不是未定项，清单里直接写死：单账户 5 次/15 分钟、全局 20 次/15 分钟、fail2ban 初值 10 次/15 分钟。异地 30 天保留拆成两条：首次部署时可验证的策略与保留设置，以及运行满 30 天后的复查。

第 12.1 节的运维面板在本轮分支整理中补齐，执行记录见 21.22。清单第 E 节仍保留它的部署验收标准：面板必须能表达「未知」，且未知不得渲染成正常。

清单本身不是执行记录。它一条都还没验过，因为 VPS 尚未购置。

### 21.22 分支收束与运维状态面板（2026-08-31）

`main` 曾把 Hanshin 设计研究的三个提交夹在后端提交之间并推到远端。此次没有重写公开历史，而是新增反向提交，只删除 Hanshin 的数据、布局、路由、样式、工具和两项专用字体依赖。后续数据库、迁移、故障矩阵与部署代码没有改动。未完成的 Hanshin、Jiege、Juanshou 研究及设计文档保存在 `codex/ui-comparison-recovery`，不再混在 `main` 的工作树里。

备份实现继续采用 `main` 已有的 `database-backup.ts` 与非破坏性恢复演练，没有把 `design/ui-restart` 上另一套同名实现整体搬来。真实文件复现确认，校验 staging 数据库会留下随机名的 `-wal` 与 `-shm`；修复后，成功与失败路径都会清理 staging 及其 sidecar。专项用例在用户叫停后续测试前已经完成红绿验证，最终为 8/8。
...
### 21.29 机器翻译的日英变体（2026-09-09）

Morii 在本轮批准了一条产品扩展：**正文只写中文，日语和英语变体交给机器翻译**。此前 `AGENTS.md` 那条「never fabricate or copy a translation」既没有允许也没有禁止机翻——它针对的是两种失败（把原文当译文端出去、编造内容填满变体），而机翻两样都不是。规则已改写：允许机翻，**但必须标注**，未标注的机翻与编造同等对待。`hreflang` 照旧声明全部变体。

#### 为什么是构建期而不是运行时

旧站 Twilight 走的是运行时路线：`twilight.config.yaml` 里 `translate.service: "client.edge"`，由 `translate.js` 在浏览器里调 Edge 的接口就地替换 DOM。它只有一份中文内容、一套路由。

那条路搬不过来。Moriium 的三语是真路由，`/ja/rss.xml` 与 `/en/rss.xml` 各自独立，搜索索引按语言在构建期生成。运行时翻译的文字不存在于构建产物里，**日英订阅者收到的仍是中文，日英读者搜不到任何东西**。因此选构建期：译文是真实文章，进同一套发布闸门、同一份 sitemap、同一个搜索索引。

#### 翻译服务：Morii 选定 Edge 接口

`https://edge.microsoft.com/translate/translatetext`，与 Twilight 同源。已向 Morii 说明它是未公开的浏览器内部接口，没有契约、没有服务端调用条款、变更不会通知；Morii 确认使用。实测记录在案：请求体必须是 `System.String[]`（纯字符串数组，不是 `[{Text}]`）；**四次开场尝试里三次 ECONNRESET**，随后一轮十二次全部成功。因此客户端固定重试五次并指数退避，`MAX_ATTEMPTS` 是这个观测的产物，不是猜的。

#### 三条设计约束

**一、保真靠「不发送」，不靠哨兵。**`src/server/translation/markdown.ts` 把正文切成有序片段，代码围栏、`mermaid`、`$$` 数学块、`:::` 容器标记行与 `::` 叶子指令全部标为不可翻译，**根本不进请求数组**。常见的哨兵替换方案依赖引擎不改写标记，改写了就静默损坏文档；没被发送的东西不可能被改坏。有一条恒等往返用例：翻译器为恒等函数时，重组结果与原文逐字节相同。

**二、失败关闭。**`translateArticleInto` 先把完整译文拿到手，再写库。服务中途失败则一行不写——**缺失的变体是站点已经会正确显示的状态（不可用），半篇译文则是一个已发布页面**。有用例证明失败后库里仍只有原文一篇。

**三、译文永远是草稿。**翻译只产出未发布版本，发布仍是原来那个操作、原来那道闸门。Morii 校对后再发布，这是「必须标注」之外的第二道保障。
...
| --- | --- |
| 文章页 | 标题下一条注记，三语各一份文案 |
| RSS 摘要 | 前置 `［机器翻译］` / `［機械翻訳］` / `[Machine translation]` |
| 后台 | 文章身份面板显示「译自 zh（机器翻译）」 |

RSS 那一条是 Morii 明确选的方案 A。阅读器只渲染标题与摘要，文章页上的注记永远到不了订阅者眼前，不标注等于那条规则在这个界面上落空。`tests/translation-label.test.mjs` 把三个界面绑在一起，新增第四个界面而不标注会在那里红。

#### 数据形状

迁移 002 给 `articles` 加 `machine_translated_from`，落在 articles 而不是 versions：它描述这个变体的来源，不描述某一次修订，和 `lang`、`slug` 同类。前言字段 `machineTranslation` 只在有值时写出——无条件写成 null 会让它出现在 Morii 亲手写的文章上，而注记读的是「有没有这个字段」。

`tests/admin-db.test.mjs` 那条把列与 `src/content-schema.ts` 绑住的用例从 14 个字段改到 15 个，仍然是加字段不加迁移就会红。备份恢复演练里写死的 `migrationVersion === 1` 改为读 `MIGRATIONS` 的末位 id——那条断言本意是「恢复出来的副本已迁移到当前版本」，写死数字会让每次加迁移都无意义地红一次。

#### 已知限制，不要当成做完了

1. **指令属性不翻译。**`:::note{title="注记"}` 的 `title` 与 `::video{title="…"}` 的标题在译文里仍是中文。翻译属性要拆引号，风险高于收益，当前由 Morii 校对时处理。
2. **句中行内代码会连同整段一起送出去。**保护的是块级结构；行内 `` ` `` 与链接目标随段落进入引擎，可能被改写。校对能发现，但这是保真的下一个改进点。
3. **没有缓存。**原文未变时重复点击会重复调用接口。ADR 讨论时提过要存译文，本轮未做。
4. **没有端到端浏览器验证。**新增的翻译按钮与译文草稿流程由处理器层用例与模板编译覆盖，未在真实浏览器里点过。
