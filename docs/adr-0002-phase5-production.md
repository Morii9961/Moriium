...
| 7 | 改写 `docs/deployment.md` 的五处（目录布局、构建位置、发布序列、systemd、Nginx 与证书） | 见第 15 节 |
...
**一、证书过期会把你自己锁在门外。**过期前 30 天必须告警，并入第 12 节的监控。同时保留一条带外恢复路径：SSH 进 VPS，临时把 `ssl_verify_client` 改成 `off`，签一张新证书后改回来。**这条恢复路径要在第 11.4 节的演练里真的走一遍**，不能只写在文档里。
...
| 健康端点（只在 Admin 主机名下，需客户端证书） | 连续 3 次失败 |
| 备份 | 任一次备份未在预期时间内成功 |
| 站点重建 | 失败，或 `published_version_id` 与 `live_version_id` 持续不一致超过 15 分钟 |
| 磁盘 | 剩余空间低于阈值 |
| 客户端证书与 CA | 任一张在 30 天内到期（见第 10.4 节） |
| 登录失败 | 单账户短时间内多次失败 |
...
1. **客户端证书在手机上的实际体验。**iOS 要装描述文件，Android 与桌面浏览器各有各的导入路径。方案本身成立，但「换新手机时要点几下」这件事只有实际做过才知道，要在 Phase 6A 里由 Morii 亲手走一遍并记下来。
...
| 9 | 改用客户端证书后，初稿没写自锁与 CA 私钥丢失两种失效 | 这两种都是「平时不会发生、发生时没有退路」。第 10.4 节补了过期告警与带外恢复路径，第 11.2 与 11.4 节把 CA 私钥纳入备份、把自锁恢复纳入演练 |
