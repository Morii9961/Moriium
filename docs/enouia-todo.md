...
- [~] 故障矩阵前三项已完成：API 断开不再暴露浏览器异常（ADR 21.14）；两个真实 SQLite 连接抢写时，文章接口返回可重试的 503，且不留下半篇文章（ADR 21.15）；第 11 块的本机备份恢复用例重跑 7/7，足以关闭本机项但不能冒充异地/VPS RTO（ADR 21.16）。草稿越权、媒体故障和静态回退还要逐项核对现有证据并补缺口。
